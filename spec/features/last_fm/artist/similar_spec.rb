...
RSpec.describe LastFM::Artist::Similar do
  ...
  describe 'successful processing' do
    context 'when artist exists' do
      let(:output) do
        VCR.use_cassette 'lastfm_artist_similar_success' do
          subject.call(artist_name: 'wild nothing')
        end
      end

      it { expect(output).to eq(Helpers::LastFM.artist_similar_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when no artist name given' do
      let(:output) do
        VCR.use_cassette 'lastfm_artist_similar_no_name' do
    ...
    context 'when wrong artist name' do
      let(:output) do
        VCR.use_cassette 'lastfm_artist_similar_wrong_name' do
