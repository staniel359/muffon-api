...
RSpec.describe YouTube::Video::Tags do
  ...
  describe 'successful processing' do
    context 'when video present' do
      let(:output) do
        VCR.use_cassette 'services/youtube/video/tags/success' do
          subject.call(video_id: '1aTIkQf3eRY')
        end
      end

      it { expect(output).to eq(Helpers::YouTube::Video.tags_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong video_id' do
      let(:output) do
        VCR.use_cassette 'services/youtube/video/tags/wrong_id' do
