RSpec.describe Spotify::Track::Links do
  ...
  describe 'successful processing' do
    context 'when track_id present' do
      let(:output) do
        VCR.use_cassette 'services/spotify/track/links/success' do
          subject.call(track_id: '29d0nY7TzCoi22XBqDQkiP')
        end
      end

      it { expect(output).to eq(Helpers::Spotify::Track.links_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong track_id' do
      let(:output) do
        VCR.use_cassette 'services/spotify/track/links/wrong_id' do
