RSpec.describe AmazonMusic::Track::Links do
  ...
  describe 'successful processing' do
    context 'when album_id and track_id present' do
      let(:output) do
        VCR.use_cassette 'services/amazonmusic/track/links/success' do
          subject.call(album_id: 'B00BRRGQAI', track_id: 'B00BRRGTQ4')
        end
      end

      it { expect(output).to eq(Helpers::AmazonMusic::Track.links_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong album_id' do
      let(:output) do
        VCR.use_cassette 'services/amazonmusic/track/links/wrong_album_id' do
    ...
    context 'when wrong track_id' do
      let(:output) do
        VCR.use_cassette 'services/amazonmusic/track/links/wrong_track_id' do
