RSpec.describe Genius::Track::Albums do
  ...
  describe 'successful processing' do
    context 'when track_id present' do
      let(:output) do
        VCR.use_cassette 'services/genius/track/albums/success' do
          subject.call(track_id: '344944', profile_id: 1)
        end
      end

      it { expect(output).to eq(Helpers::Genius::Track.albums_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong track_id' do
      let(:output) do
        VCR.use_cassette 'services/genius/track/albums/wrong_id' do
