RSpec.describe LastFM::Track::Links do
  ...
  describe 'successful processing' do
    context 'when track exists' do
      let(:output) do
        VCR.use_cassette(
          'services/lastfm/track/links/success'
        ) do
          subject.call(
      ...
      it { expect(output).to match_hash(lastfm_track_links_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong artist' do
      let(:output) do
        VCR.use_cassette(
          'services/lastfm/track/links/wrong_artist'
        ) do
          subject.call(
    ...
    context 'when wrong title' do
      let(:output) do
        VCR.use_cassette(
          'services/lastfm/track/links/wrong_title'
        ) do
          subject.call(
            artist_name: 'wild nothing',
