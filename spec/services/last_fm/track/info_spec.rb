RSpec.describe LastFM::Track::Info do
  ...
  describe 'successful processing' do
    context 'when artist and track exists' do
      let(:output) do
        VCR.use_cassette(
          'services/lastfm/track/info/success'
        ) do
          subject.call(
      ...
      it { expect(output).to match_hash(lastfm_track_info_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong track title' do
      let(:output) do
        VCR.use_cassette(
          'services/lastfm/track/info/wrong_title'
        ) do
          subject.call(
            artist_name: 'kate bush',
    ...
    context 'when wrong artist name' do
      let(:output) do
        VCR.use_cassette(
          'services/lastfm/track/info/wrong_name'
        ) do
          subject.call(
