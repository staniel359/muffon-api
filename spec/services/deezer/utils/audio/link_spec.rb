RSpec.describe Deezer::Utils::Audio::Link do
  ...
  describe 'successful processing' do
    context 'when track_id present' do
      let(:output) do
        VCR.use_cassette 'services/deezer/utils/audio/link/success' do
          subject.call(track_id: '69000435')
        end
      end

      it { expect(output).to eq(Helpers::Deezer::Utils.audio_link) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong track_id' do
      let(:output) do
        VCR.use_cassette 'services/deezer/utils/audio/link/wrong_id' do
