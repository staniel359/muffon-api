RSpec.describe MusixMatch::Track::Info do
  ...
  describe 'successful processing' do
    context 'when track_id present' do
      let(:output) do
        VCR.use_cassette 'services/musixmatch/track/info/success' do
          subject.call(track_id: '6717709', profile_id: 1)
        end
      end

      it { expect(output).to eq(Helpers::MusixMatch::Track.info_data) }
    end
  end
  ...
  describe 'no processing' do
    ...
    context 'when wrong track_id' do
      let(:output) do
        VCR.use_cassette 'services/musixmatch/track/info/wrong_id' do
